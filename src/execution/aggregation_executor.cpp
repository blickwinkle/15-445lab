...
AggregationExecutor::AggregationExecutor(ExecutorContext *exec_ctx, const AggregationPlanNode *plan,
    ...
    : AbstractExecutor(exec_ctx), plan_(plan), child_(std::move(child)), aht_(plan->aggregates_, plan->agg_types_), aht_iterator_(nullptr) {}
...
auto AggregationExecutor::Next(Tuple *tuple, RID *rid) -> bool { 
    ...
    merged_value.insert(merged_value.end(), aht_iterator_->Key().group_bys_.begin(), aht_iterator_->Key().group_bys_.end());
    merged_value.insert(merged_value.end(), aht_iterator_->Val().aggregates_.begin(), aht_iterator_->Val().aggregates_.end());
